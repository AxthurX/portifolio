...
function FInput({ id, ...rest }: FInputProps) {
	return (
		<input
			id={id}
			className='w-full border-border border-b bg-transparent py-3 text-base text-foreground transition-all duration-300 placeholder:text-muted-foreground/50 focus:border-primary focus:outline-none'
			{...rest}
		/>
	);
}
...
function FTextarea({ id, rows = 4, ...rest }: FTextareaProps) {
	return (
		<textarea
			id={id}
			rows={rows}
			className='w-full resize-none border-border border-b bg-transparent py-3 text-base text-foreground leading-relaxed transition-all duration-300 placeholder:text-muted-foreground/50 focus:border-primary focus:outline-none'
			{...rest}
		/>
	);
}
		...
		<div className='flex flex-wrap gap-2 pt-2'>
			{BUDGETS.map((b) => (
				<button
					...
					className={`rounded-full border px-4 py-2 font-medium text-xs transition-all duration-200 ${
						value === b
					...
					{!selected ? (
						...
						>
							<motion.button
								...
								className='group mb-10 flex items-center gap-2 font-medium text-muted-foreground text-sm transition-colors hover:text-foreground'
							>
								<ArrowLeft
