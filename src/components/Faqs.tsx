...
const certificates: Certificate[] = [
	{
		...
		date: 'Abr 2026',
		credentialUrl: '#',
		skills: ['Gestao de Projetos', 'Agile', 'Scrum'],
		size: 'large',
		color: 'gold',
	},
	{
	...
	},
	{
		...
		credentialUrl: '#',
		skills: ['Scrum', 'Gestao de Projetos'],
		size: 'medium',
		color: 'silver',
	},
	{
...
const sizeConfig = {
	large:  { cupH: 'h-20', cupW: 'w-16', stemH: 'h-8',  baseW: 'w-20', cardW: 'col-span-2 row-span-2', minH: 'min-h-[260px]' },
	medium: { cupH: 'h-14', cupW: 'w-12', stemH: 'h-5',  baseW: 'w-16', cardW: 'col-span-1 row-span-2', minH: 'min-h-[200px]' },
	small:  { cupH: 'h-10', cupW: 'w-9',  stemH: 'h-4',  baseW: 'w-12', cardW: 'col-span-1 row-span-1', minH: 'min-h-[160px]' },
	mini:   { cupH: 'h-7',  cupW: 'w-7',  stemH: 'h-3',  baseW: 'w-10', cardW: 'col-span-1 row-span-1', minH: 'min-h-[140px]' },
};

function TrophySVG({ size, color }: { size: TrophySize; color: Certificate['color'] }) {
	...
	return (
		<div className='flex flex-col items-center'>
			...
			<div className={`relative ${s.cupH} ${s.cupW} ${c.cup}`}>
				<svg viewBox="0 0 64 72" fill="currentColor" className='h-full w-full drop-shadow-lg'>
					{/* Handles */}
					<path d="M4 14 Q0 14 0 22 Q0 30 6 32 L10 30 Q6 28 6 22 Q6 18 8 16 Z" opacity="0.7" />
					<path d="M60 14 Q64 14 64 22 Q64 30 58 32 L54 30 Q58 28 58 22 Q58 18 56 16 Z" opacity="0.7" />
					{/* Main cup */}
					<path d="M8 6 Q8 0 32 0 Q56 0 56 6 L52 42 Q50 52 32 54 Q14 52 12 42 Z" />
					{/* Shine */}
					<path d="M14 8 Q16 4 24 3 L22 20 Q16 18 14 8 Z" fill="white" opacity="0.15" />
					{/* Star or detail for large */}
					{size === 'large' && (
						<path d="M32 18 L34 24 L40 24 L35 28 L37 34 L32 30 L27 34 L29 28 L24 24 L30 24 Z" fill="white" opacity="0.3" />
					)}
				</svg>
			</div>
			{/* Stem */}
...
function CertificateCard({ cert, index }: { cert: Certificate; index: number }) {
	...
	return (
		...
		>
			<div
				className={`relative h-full cursor-pointer overflow-hidden rounded-xl border p-4 transition-all duration-300 flex flex-col justify-between ${c.base} ${hovered ? `shadow-lg ${c.glow}` : 'shadow-sm'}`}
				onMouseEnter={() => setHovered(true)}
				onMouseLeave={() => setHovered(false)}
			>
				{/* Shelf glow top line */}
				<div className={`absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-current to-transparent ${c.cup} opacity-30`} />

				{/* Trophy */}
				<div className={`flex justify-center transition-transform duration-300 ${hovered ? '-translate-y-1' : ''}`}>
					<TrophySVG size={cert.size} color={cert.color} />
				</div>

				{/* Info */}
				<div className='mt-3 flex flex-col gap-1'>
					<p className={`font-semibold leading-tight text-foreground ${cert.size === 'large' ? 'text-sm' : cert.size === 'mini' ? 'text-[10px]' : 'text-xs'}`}>
						{cert.title}
					</p>
					<p className={`${c.label} font-medium ${cert.size === 'mini' ? 'text-[9px]' : 'text-[10px]'}`}>
						{cert.issuer}
					</p>
					...
					{cert.skills && cert.size !== 'mini' && (
						<div className='mt-1 flex flex-wrap gap-1'>
							{cert.skills.slice(0, cert.size === 'large' ? 3 : 1).map((skill) => (
								<span
									key={skill}
									className={`rounded-full border px-1.5 py-0.5 text-[8px] font-medium ${c.badge}`}
								>
									{skill}
								</span>
							))}
			...
			<div className='relative z-10 px-4 md:px-0'>
				...
				<div className='mb-12 md:mb-16'>
					...
					<p className='mt-4 max-w-xl text-muted-foreground text-sm leading-relaxed md:text-base'>
						Uma vitrine de aprendizado continuo — cada certificado representa uma nova habilidade conquistada.
					</p>
				</div>
				...
				<div className='relative mx-auto max-w-5xl'>
					...
					<div className='relative rounded-2xl border border-border bg-card/30 p-1 shadow-2xl backdrop-blur-sm'>
						{/* Top bar with lights */}
						<div className='flex items-center gap-3 rounded-t-xl border-b border-border bg-card/60 px-6 py-3'>
							<div className='h-2 w-2 rounded-full bg-yellow-400/70 shadow-sm shadow-yellow-400/50' />
							<div className='h-2 w-2 rounded-full bg-yellow-300/60 shadow-sm shadow-yellow-300/50' />
							<div className='h-2 w-2 rounded-full bg-yellow-200/50 shadow-sm shadow-yellow-200/50' />
							<div className='ml-auto text-[10px] font-medium uppercase tracking-widest text-muted-foreground'>
								Vitrine de Conquistas
							</div>
						</div>

						{/* Cabinet glass interior */}
						<div className='relative rounded-b-xl bg-gradient-to-b from-card/40 to-background/60 p-6'>
							{/* Shelf lines */}
					...
					{/* Cabinet legs/base */}
					<div className='mx-auto mt-1 h-3 w-[96%] rounded-b-lg border-x border-b border-border/50 bg-card/30' />
					<div className='mx-auto mt-0.5 h-2 w-[92%] rounded-b-lg border-x border-b border-border/30 bg-card/20' />
				</div>
				...
				>
					...
					<span className='rounded-full border border-border bg-card px-4 py-1.5 text-xs font-medium text-muted-foreground'>
						{certificates.length} certificados e licencas
					</span>
					<div className='h-px flex-1 bg-border' />
				</motion.div>
			</div>
